...
static t_vec4d	calc_transformed_point(const t_primitive *primitive,
...
static t_bool	is_cutted(const t_primitive *primitive,
const t_limitation *limit, const t_vec4d *global_point)
{
	...
	point = limit->is_relative ?
	calc_transformed_point(primitive, global_point) : *global_point;
...
const t_ray *ray, float *target_t, const float t)
{
	...
	if (has_next(&limit_iter))
	{
		...
		{
			if (is_cutted(primitive,
			limitation_next(&limit_iter), &global_point))
				return (FALSE);
		}
	}
	*target_t = t;
