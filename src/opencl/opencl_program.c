...
#define CL_BUILD_FLAGS	"-cl-single-precision-constant -cl-std=CL1.2"
#define BUILD_OPTIONS	"-I " RT_CWD "/src/opencl/kernel " CL_BUILD_FLAGS
...
static void			build_program(t_opencl_program *clprogram)
{
	...
	err = clBuildProgram(clprogram->program, 0, NULL,
...
const char *kernel_name)
{
	...
	if (source == NULL)
		log_fatal("Failed to read openCL source file...",
		RT_FILE_OPENING_ERROR);
	...
	if (clprogram.kernel == NULL || err != CL_SUCCESS)
		log_fatal(opencl_get_error(err), RT_OPENCL_ERROR);
	clprogram.outputbuffer = clCreateBuffer(g_clcontext.context,
