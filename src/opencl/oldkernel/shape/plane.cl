...
{
	return ((t_vec4)(primitive->orientation.s4, primitive->orientation.s5, primitive->orientation.s6, 0.0f));
}
...
t_bool	plane_intersected(__constant t_primitive *primitive, const t_ray *ray, t_scalar *t)
{
	...
	t_scalar denom = -dot(ray->direction, plane_normal);
	t_scalar sign = 1.0f;
	if (denom < 0.0f)
	{
		denom = -denom;
...
inline t_vec4	obtain_plane_normal(const t_vec4 *point, __constant t_primitive *primitive)
{
