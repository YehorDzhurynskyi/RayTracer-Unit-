...
{
	*u = 0.5f + atan2(normal->y, normal->x) / (M_PI * 2.0f);
	*v = 0.5f + asin(normal->z) / M_PI;
}

static void	plane_obtain_uv(__constant t_primitive *primitive,
const t_vec4 *point, t_scalar *u, t_scalar *v)
{
	const t_vec4 new_point = mat4x4_mult_vec4(primitive->orientation, *point);
...
const t_vec4 *point, const t_vec4 *normal, t_scalar *u, t_scalar *v)
{
	...
	const t_vec4 to_point = *point - primitive->position;
	const t_scalar height_factor = dot(to_point, cyl_dir);
	const t_scalar arccos = acos(dot(*normal, x_basis));
...
const t_vec4 *point, const t_vec4 *normal, t_scalar *u, t_scalar *v)
{
	if (primitive->primitive_type == SPHERE)
		sphere_obtain_uv(primitive, point, normal, u, v);
	else if (primitive->primitive_type == PLANE)
		plane_obtain_uv(primitive, point, u, v);
	else if (primitive->primitive_type == CYLINDER)
		cylinder_obtain_uv(primitive, point, normal, u, v);
