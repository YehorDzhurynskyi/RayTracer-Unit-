...
{
	t_vec4 p = ray->origin + ray->direction * t;
	p -= shape->position;
	return (dot(p, cone->direction) <= 0.0);
}
...
{
	t_bool b1 = t1 < 0.0 || is_cutted_cone(ray, shape, cone, t1);
	t_bool b2 = t2 < 0.0 || is_cutted_cone(ray, shape, cone, t2);
	if (b1 && b2)
		return (-1.0);
	if (b1)
		return (t2);
