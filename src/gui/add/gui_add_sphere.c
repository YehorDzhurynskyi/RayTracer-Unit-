...
void	gui_add_sphere(void)
{
	...
	sphere = scenebuffer_default_sphere();
	primitive.primitive_type = SPHERE;
	scenebuffer_map(&g_main_scene, g_main_scene.meta.shapes_size,
	sizeof(t_shape) + sizeof(t_primitive) + sizeof(t_sphere), SHAPEBUFF_TARGET);
	scenebuffer_add_shape(&g_main_scene, &shape, FALSE);
	scenebuffer_add_primitive(&g_main_scene, &primitive, &sphere, SHAPEBUFF_TARGET);
	scenebuffer_unmap(&g_main_scene, SHAPEBUFF_TARGET);
	log_notify("Sphere was added to scene");
}
