...
const t_lightsource *new_lightsource)
{
	...
	up_to_date = ft_memcmp(old_lightsource,
		new_lightsource, sizeof(t_lightsource)) == 0;
	if (up_to_date)
		return ;
	scenebuffer_map(&g_main_scene, new_lightsource->addr,
	sizeof(t_lightsource), LIGHTSOURCEBUFF_TARGET);
	scenebuffer_put(&g_main_scene,
		(void*)new_lightsource, 0, sizeof(t_lightsource));
...
static void	render_actual_lightsource_form(const t_lightsource *lightsource)
{
	...
	if (lightsource->lightsource_type == POINTLIGHT)
		render_pointlightsource_form(lightsource,
			(const t_pointlightsource*)actual);
	else if (lightsource->lightsource_type == DIRLIGHT)
		render_dirlightsource_form(lightsource,
			(const t_dirlightsource*)actual);
	else if (lightsource->lightsource_type == SPOTLIGHT)
		render_spotlightsource_form(lightsource,
			(const t_spotlightsource*)actual);
}
...
void		render_lightsource_form(const t_lightsource *lightsource)
{
	...
	if (nk_tree_push(g_nk_context, NK_TREE_TAB, "Lightsource", NK_MAXIMIZED))
	{
		...
		nk_label_colored(g_nk_context,
			"intensity", NK_TEXT_LEFT, property_color);
		nk_property_float(g_nk_context,
			"#value", 0.0f, &intensity, 1.0f, 0.1f, 0.1f);
