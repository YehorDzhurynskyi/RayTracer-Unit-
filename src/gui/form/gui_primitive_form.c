...
static void	render_primitive_orientation(t_primitive *primitive)
{
	...
	rotations.z = -atan2(-primitive->orientation.s4, primitive->orientation.s0) *
		(180.0f / M_PI);
	nk_layout_row_dynamic(g_nk_context, 30, 1);
	nk_label_colored(g_nk_context, "orientation", NK_TEXT_LEFT, property_color);
	nk_property_float(g_nk_context, "#rotX:", -180.0f, &rotations.x, 180.0f, 1.0f, 1.0f);
	nk_property_float(g_nk_context, "#rotY:", -180.0f, &rotations.y, 180.0f, 1.0f, 1.0f);
	nk_property_float(g_nk_context, "#rotZ:", -180.0f, &rotations.z, 180.0f, 1.0f, 1.0f);
...
const t_primitive *new_primitive)
{
	...
	up_to_date = ft_memcmp(old_primitive, new_primitive, sizeof(t_primitive)) == 0;
	if (up_to_date)
		return ;
	scenebuffer_map(&g_main_scene, shape->addr + sizeof(t_shape),
	sizeof(t_primitive), SHAPEBUFF_TARGET);
	scenebuffer_put(&g_main_scene, (void*)new_primitive, 0, sizeof(t_primitive));
