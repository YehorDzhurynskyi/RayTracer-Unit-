...
const t_pointlightsource *new_pointlightsrc)
{
	...
	up_to_date = ft_memcmp(old_pointlightsrc, new_pointlightsrc, sizeof(t_pointlightsource)) == 0;
	if (up_to_date)
		return ;
	scenebuffer_map(&g_main_scene, lightsource->addr + sizeof(t_lightsource),
	sizeof(t_pointlightsource), LIGHTSOURCEBUFF_TARGET);
	scenebuffer_put(&g_main_scene, (void*)new_pointlightsrc, 0, sizeof(t_pointlightsource));
