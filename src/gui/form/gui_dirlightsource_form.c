...
const t_dirlightsource *new_dirlightsrc)
{
	...
	up_to_date = ft_memcmp(old_dirlightsrc, new_dirlightsrc, sizeof(t_dirlightsource)) == 0;
	if (up_to_date)
		return ;
	scenebuffer_map(&g_main_scene, lightsource->addr + sizeof(t_lightsource),
	sizeof(t_dirlightsource), LIGHTSOURCEBUFF_TARGET);
	scenebuffer_put(&g_main_scene, (void*)new_dirlightsrc, 0, sizeof(t_dirlightsource));
...
const t_dirlightsource *dirlightsrc)
{
	...
	if (nk_tree_push(g_nk_context, NK_TREE_TAB, "Parallel lightsource", NK_MAXIMIZED))
	{
