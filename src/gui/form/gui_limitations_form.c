...
static void	render_limitation_item(const t_limitation *limitation,
	int tree_item_id)
{
	char	caption[255];
	size_t	name_len;
	...
	ft_strncpy(caption,
		g_limitation_captions[limitation->limitation_type - 1], 255);
	...
	const t_primitive *primitive)
{
	...
	if (nk_tree_push(g_nk_context, NK_TREE_TAB, "Limitations", NK_MAXIMIZED))
	{
		if (nk_button_symbol_label(g_nk_context, NK_SYMBOL_PLUS,
			"Add limitation", NK_TEXT_ALIGN_RIGHT))
		{
		}
		i = 0;
...
const t_buff_target buff_target)
{
	...
	up_to_date = ft_memcmp(old_limitation,
		new_limitation, sizeof(t_limitation)) == 0;
	if (up_to_date)
		return ;
	scenebuffer_map(&g_main_scene, addr_offset,
	sizeof(t_limitation), buff_target);
	scenebuffer_put(&g_main_scene, (void*)new_limitation, 0,
		sizeof(t_limitation));
...
const t_claddress addr_offset, const t_buff_target buff_target)
{
	...
	if (nk_tree_push(g_nk_context, NK_TREE_TAB, "Limitation", NK_MAXIMIZED))
	{
		...
		update_limitation(limitation, &new_limitation,
			addr_offset, buff_target);
		nk_tree_pop(g_nk_context);
	}
}
