...
const t_spotlightsource *new_spotlightsrc)
{
	...
	up_to_date = ft_memcmp(old_spotlightsrc,
		new_spotlightsrc, sizeof(t_spotlightsource)) == 0;
	if (up_to_date)
		return ;
	scenebuffer_map(&g_main_scene, lightsource->addr + sizeof(t_lightsource),
	sizeof(t_spotlightsource), LIGHTSOURCEBUFF_TARGET);
	scenebuffer_put(&g_main_scene, (void*)new_spotlightsrc,
	0, sizeof(t_spotlightsource));
