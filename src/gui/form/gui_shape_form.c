...
void				render_children(const t_shape *shape)
{
	...
	{
		if (nk_button_symbol_label(g_nk_context, NK_SYMBOL_PLUS, "Add child", NK_TEXT_ALIGN_RIGHT))
		{
...
void				render_shape_form(const t_shape *shape)
{
	...
	if (nk_tree_push(g_nk_context, NK_TREE_TAB, "Shape", NK_MAXIMIZED))
	{
		...
		nk_label_colored(g_nk_context, "relation type", NK_TEXT_LEFT, property_color);
		new_shape.relation_type = 1 + nk_combo(g_nk_context, g_relation_type_names, 3,
		shape->relation_type - 1, 30, nk_vec2(190, 200));
		if (shape->material_addr == DEFAULT_MATERIAL_ID)
		{
			nk_layout_row_dynamic(g_nk_context, 30, 1);
			if (nk_button_symbol_label(g_nk_context, NK_SYMBOL_PLUS, "Set material", NK_TEXT_ALIGN_RIGHT))
				render_chooser = TRUE;
			if (render_chooser)
			{
