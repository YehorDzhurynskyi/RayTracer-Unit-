...
void					gui_select_object(t_claddress addr,
	const t_buff_target target)
{
	...
	if (target == SHAPEBUFF_TARGET && addr != NONE_SELECTED_ADDR)
	{
		...
		g_selected_object = (const t_shape*)(g_main_scene.host_shapebuffer +
			addr);
	}
	else if (target == LIGHTSOURCEBUFF_TARGET)
		g_selected_object = (const t_lightsource*)
			(g_main_scene.host_lightsourcebuffer + addr);
	else if (target == MATERIALBUFF_TARGET)
		g_selected_object = (const t_material*)
			(g_main_scene.host_materialbuffer + addr);
	else if (target == TEXTUREBUFF_TARGET)
		g_selected_object = (const t_material*)
			(g_main_scene.host_texturebuffer + addr);
}
...
{
	if (nk_begin(g_nk_context, obtain_caption(),
		nk_rect(OBJ_X, OBJ_Y, OBJ_WIDTH, OBJ_HEIGHT),
