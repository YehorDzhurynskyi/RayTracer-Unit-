...
const t_chess_texture *new_chess_texture)
{
	...
	up_to_date = ft_memcmp(old_chess_texture, new_chess_texture, sizeof(t_chess_texture)) == 0;
	if (up_to_date)
		return ;
	scenebuffer_map(&g_main_scene, texture->addr + sizeof(t_texture),
	sizeof(t_chess_texture), TEXTUREBUFF_TARGET);
	scenebuffer_put(&g_main_scene, (void*)new_chess_texture, 0, sizeof(t_chess_texture));
...
void		render_chess_texture_form(const t_texture *texture)
{
	...
	if (nk_tree_push(g_nk_context, NK_TREE_TAB, "Chessboard texture", NK_MAXIMIZED))
	{
		...
		nk_label_colored(g_nk_context, "First color", NK_TEXT_LEFT, property_color);
		gui_render_color_picker(&color1);
		nk_label_colored(g_nk_context, "Second color", NK_TEXT_LEFT, property_color);
		gui_render_color_picker(&color2);
		new_chess_texture.color1 = from_nkcolor(color1);
		new_chess_texture.color2 = from_nkcolor(color2);
		update_chess_texture(texture, (t_chess_texture*)(texture + 1), &new_chess_texture);
