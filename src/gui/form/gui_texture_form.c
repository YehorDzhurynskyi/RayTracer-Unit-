...
static void	update_texture(const t_texture *old_texture, const t_texture *new_texture)
{
	t_bool	up_to_date;
	...
	if (up_to_date)
		return ;
	scenebuffer_map(&g_main_scene, new_texture->addr, sizeof(t_texture), TEXTUREBUFF_TARGET);
...
void		render_texture_form(const t_texture *texture)
{
	...
	if (nk_tree_push(g_nk_context, NK_TREE_TAB, "Texture", NK_MAXIMIZED))
	{
		...
		nk_property_float(g_nk_context, "#value", 1.0f, &scale, 20.0f, 0.1f, 0.1f);
		nk_label_colored(g_nk_context, "U Offset", NK_TEXT_LEFT, property_color);
		nk_property_float(g_nk_context, "#value", 0.0f, &uoffset, 1.0f, 0.01f, 0.01f);
		nk_label_colored(g_nk_context, "V Offset", NK_TEXT_LEFT, property_color);
		nk_property_float(g_nk_context, "#value", 0.0f, &voffset, 1.0f, 0.01f, 0.01f);
