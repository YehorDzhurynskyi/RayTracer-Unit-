...
void	render_climage_texture_form(const t_texture *texture)
{
	...
	if (nk_tree_push(g_nk_context, NK_TREE_TAB,
		"Resource texture", NK_MAXIMIZED))
	{
		nk_label_colored(g_nk_context, "Texture name", NK_TEXT_LEFT,
			property_color);
		texture_name = texture_map_at(&g_main_scene.texture_map,
			climage_texture->tex_index);
