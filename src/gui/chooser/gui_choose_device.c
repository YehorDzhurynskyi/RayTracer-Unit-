...
static int		choose_devices(int ndevices)
{
	...
	ndevices = ndevices > RT_MAX_CL_DEVICE_COUNT ?
				RT_MAX_CL_DEVICE_COUNT : ndevices;
	...
	while (++i < ndevices)
	{
		clGetDeviceInfo(devices[i], CL_DEVICE_NAME,
			RT_MAX_CL_DEVICE_NAME_LEN, device_name, NULL);
...
static t_bool	popup(int ndevices)
{
	...
	if (nk_popup_begin(g_nk_context, NK_POPUP_STATIC, "Device", 0,
		nk_rect(DEV_CHOOSE_POP_X, DEV_CHOOSE_POP_Y,
			DEV_CHOOSE_POP_WIDTH, 50 * ndevices + 55)))
	{
		nk_layout_row_dynamic(g_nk_context, 30, 1);
		nk_label(g_nk_context,
		"Choose OpenCL device to compile raytracer kernel", NK_TEXT_RIGHT);
		active = choose_devices(ndevices);
		if (nk_button_label(g_nk_context, "Compile") && active >= 0)
		{
			render_widget = FALSE;
			gui_popup_stop();
			pthread_create(&renderer_init_thread, NULL,
				(void*(*)(void*))renderer_init, NULL);
...
void			gui_choose_opencl_device(void)
{
	...
	if (render_widget == FALSE)
		return ;
	gui_popup_start();
	err = clGetDeviceIDs(NULL, CL_DEVICE_TYPE_ALL,
		0, NULL, (unsigned int*)&ndevices);
	if (err != CL_SUCCESS)
	{
		window_error(RT_APP_NAME " OpenCL error",
			"No device that support OpenCL");
		log_fatal(opencl_get_error(err), RT_OPENCL_ERROR);
	}
	render_widget = popup(ndevices);
}
