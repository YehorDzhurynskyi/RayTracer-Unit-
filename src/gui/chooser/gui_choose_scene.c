...
static t_bool		render_pop_scenes(const char *scenenames, int scenesnum)
{
	...
	nk_popup_begin(g_nk_context, NK_POPUP_STATIC, "Choose scene file", 0,
		nk_rect(SCENE_CHOOSE_POP_X, SCENE_CHOOSE_POP_Y,
		SCENE_CHOOSE_POP_WIDTH, ((scenesnum + 1) / 2) * 30 + 65));
	...
	{
		render_widget = FALSE;
		pthread_create(&scene_change_thread, NULL,
		(void*(*)(void*))scene_change, (void*)choosen_scene_name);
