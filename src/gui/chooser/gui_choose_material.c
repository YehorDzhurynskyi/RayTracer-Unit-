...
t_bool					return_material(const t_material **dest_material,
const t_material *selected_material)
{
...
t_bool					gui_choose_material(const t_material **dest_material)
{
	...
	nk_popup_begin(g_nk_context, NK_POPUP_STATIC, "Choose material", 0,
		nk_rect(MATERIAL_CHOOSE_POP_X, MATERIAL_CHOOSE_POP_Y - pop_height,
		MATERIAL_CHOOSE_POP_WIDTH, pop_height));
	...
	render_widget = return_material(dest_material, selected_material);
