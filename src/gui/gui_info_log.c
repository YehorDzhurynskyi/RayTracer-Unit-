...
{
	ft_strncpy(g_infolog_buffer + g_infolog_size,
		message, INFOLOG_BUFFER_SIZE - g_infolog_size);
	g_infolog_size += INFOLOG_BUFFER_SIZE - g_infolog_size >= size ?
		size : INFOLOG_BUFFER_SIZE - g_infolog_size;
}

void			gui_info_log_error(const char *message,
	const char *err_code_message)
{
	size_t	message_len;
	...
	if (12 + message_len + err_code_message_len + g_infolog_size >=
		INFOLOG_BUFFER_SIZE)
	{
...
void			gui_render_info_log(void)
{
	...
	if (nk_begin(g_nk_context, "Information log", nk_rect(LOG_X, LOG_Y,
		LOG_WIDTH, LOG_HEIGHT), g_nk_window_flags | NK_WINDOW_TITLE))
	{
