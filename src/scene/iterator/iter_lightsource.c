...
{
	return ((t_iterator){scene->meta.nlightsources,
	scene->host_lightsourcebuffer});
}

const t_lightsource	*lightsource_next(t_iterator *iterator)
{
	const t_lightsource	*current;
	t_iterator			limitation_iter;

	current = (const t_lightsource*)iterator->current;
	iterator->current = lightsource_skip(current);
	if (current->lightsource_type == POINTLIGHT
	|| current->lightsource_type == SPOTLIGHT)
	{
		limitation_iter = limitation_begin(
			(const t_primitive*)iterator->current);
