...
const t_vec3d *position, const t_vec3d *up)
{
	...
	forward = deserialize_vec3d_required(cson_valueof(cson, CSON_FORWARD_KEY), TRUE);
	right = deserialize_vec3d_required(cson_valueof(cson, CSON_RIGHT_KEY), TRUE);
...
t_camera		deserialize_camera(const t_cson *cson)
{
	...
	position = deserialize_vec3d_required(cson_valueof(cson, CSON_POSITION_KEY), FALSE);
