...
static t_clmat4x4	orientation_matrix(const t_clscalar *r)
{
...
t_clmat4x4			deserialize_orientation_required(const t_cson *cson)
{
	...
	rotation = deserialize_vec3_required(cson_valueof(cson,
	CSON_ORIENTATION_KEY), FALSE);
...
const t_vec3d default_angles)
{
	...
	rotation = deserialize_vec3_optional(cson_valueof(cson,
	CSON_ORIENTATION_KEY), FALSE, default_angles);
	return (to_orientation_matrix(rotation.x, rotation.y, rotation.z));
}
