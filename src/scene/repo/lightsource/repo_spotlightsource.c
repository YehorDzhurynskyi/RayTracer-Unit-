...
t_spotlightsource	deserialize_spotlightsource(const t_cson *cson)
{
	...
	spotlightsource.direction = deserialize_vec3_required(cson_valueof(cson,
	CSON_DIRECTION_KEY), TRUE);
	spotlightsource.attenuation = deserialize_vec3_optional(cson_valueof(cson,
	CSON_ATTENUATION_KEY), FALSE,
	(t_vec3d){1.0, 0.0, 0.0});
	spotlightsource.cosangle = deserialize_real_required(cson_valueof(cson,
	CSON_ANGLE_KEY));
