...
{
	return (validate_real_required_ranged(cson, key,
	(double[2]){-DBL_MAX, DBL_MAX}));
}

t_err_code	validate_real_optional(const t_cson *cson, const char *key,
const char *default_value_str)
{
	return (validate_real_optional_ranged(cson, key,
	(double[2]){-DBL_MAX, DBL_MAX}, default_value_str));
}
...
const double range[2], const char *default_value_str)
{
	...
	{
		return (validation_failed(real_cson,
	...
	{
		return (validation_failed(real_cson,
		RT_WRONG_VALUE_RANGE_ERROR, RANGE_MSG));
	}
	return (RT_NO_ERROR);
}
