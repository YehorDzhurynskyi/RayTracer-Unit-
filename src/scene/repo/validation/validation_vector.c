...
t_err_code			validate_vec3_required(const t_cson *cson,
const char *key, t_bool normalize)
{
...
static t_err_code	validate_normalization(const t_cson *vec_cson)
{
	t_vec3d	value;
	...
	{
		return (validation_failed(vec_cson,
		RT_ZERO_LENGTH_VECTOR_ERROR, NORM_MSG));
	}
	return (RT_NO_ERROR);
}
...
t_bool normalize, const char *default_value_str)
{
	...
	{
		return (validation_failed(vec_cson,
		RT_WRONG_DIMENSION_ERROR, DIMEN_MSG));
	}
	return (normalize ? validate_normalization(vec_cson) : RT_NO_ERROR);
}
