...
static t_err_code	validate_material_glossiness(const t_cson *cson)
{
	...
	|| (real_cson != NULL && tex_cson != NULL))
		return (validation_failed(cson, RT_NO_REQUIRED_VALUE_ERROR, GLOSS_MSG));
	if (real_cson != NULL)
		return (validate_real_required_ranged(cson, CSON_GLOSSINESS_KEY, (double[2]){0.0, 1.0}));
...
t_err_code			validate_material(const t_cson *cson)
{
	...
	err = validate_material_color(cson, CSON_DIFFUSE_COLOR_KEY, CSON_DIFFUSE_TEXTURE_KEY, DIFFUSE_MSG);
	err |= validate_material_color(cson, CSON_SPECULAR_COLOR_KEY, CSON_SPECULAR_TEXTURE_KEY, SPEC_MSG);
	err |= validate_material_glossiness(cson);
	err |= validate_real_optional_ranged(cson, CSON_IOR_KEY, (double[2]){0.0, 2.0}, IOR_STR);
	return (err);
}
