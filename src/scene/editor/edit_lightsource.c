...
static size_t	lightsource_size(t_lightsource_type type)
{
	if (type == DIRLIGHT)
...
t_lightsource *lightsource, const void *actual_lightsource)
{
	...
	scenebuffer_append(scene, lightsource,
	sizeof(t_lightsource), LIGHTSOURCEBUFF_TARGET);
	scenebuffer_append(scene, actual_lightsource,
	lightsource_size(lightsource->lightsource_type), LIGHTSOURCEBUFF_TARGET);
	scene->meta.nlightsources = scene->meta.nlightsources + 1;
}
